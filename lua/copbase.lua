...
Hooks:PreHook(CopBase, "post_init", "DS_BW_CopBase_post_init", function(self)
	...
	local weapon_mapping = DS_BW.weapon_mapping or {
	...
	if weapon_swap then
		...
		local weapon_tree_level = DS_BW._low_spawns_manager.level
		local escapes = {
			"escape_overpass",
		...
		if Global.level_data and table.contains(escapes, Global.level_data.level_id) then
			weapon_tree_level = weapon_tree_level + 2
		end
		if not weapon_swap[weapon_tree_level] then
			while not weapon_swap[weapon_tree_level] do
				weapon_tree_level = weapon_tree_level - 1
			end
		end
		local candidate = 69
		for wpn, chance in pairs(weapon_swap[weapon_tree_level]) do
			if roll < chance and candidate > chance then
				candidate = chance
