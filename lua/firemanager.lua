	...
	for _, hit_body in ipairs(bodies) do
		...
		if ray_hit then
			...
			if character then
				...
				-- if original damage was this specific, add new var to the action data that copdamage.lua file recieves, to avoid civ and cop damage
				-- all other parts of this function are vanilla.
				-- honestly i dont even know why in vanilla, damage is made to be at least 1 here (@149), instead of, i dont know, in copdamage file for example?
				if not DS_BW.firetrap_ids then
					DS_BW.firetrap_ids = {}
				end
				-- ignore unit is the projectile of the firetrap grenade, because grenade needs to ignore itself. very handy
				if dmg == -0.69 or (params.dot_data.name == "proj_launcher_incendiary" and DS_BW and DS_BW.firetrap_ids[params.ignore_unit:id()]) then
					action_data.DS_BW_cop_molotov = true
				end
